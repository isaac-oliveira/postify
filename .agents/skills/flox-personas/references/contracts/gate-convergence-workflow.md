# Gate convergence workflow

Shared workflow for `$flox-pentest`, `$flox-quality`, and `$flox-release`. It
prevents a gate from widening its confirmed scope or returning to an unbounded
correction loop. The workflow is per `work_item_id`; it does not add state to
`status.yaml`.
...
- This workflow governs the shared convergence mechanics only. Each gate keeps
  its own roadmap-defined preconditions, scope, methods, evidence, approvals,
  result, and recovery rules. Release also keeps its completion roll-up.
...
```text
## <gate> convergence ledger
gate: <pentest | quality | release>
...
Use only the applicable `findings` or `criteria` collection for the gate, as
declared by its confirmed roadmap. Do not invent entries for a gate with no
applicable collection. Keep stable IDs and prior states. `open` findings and
`failed` mandatory criteria block. A finding marked `fixed` or `accepted` is
terminal and is never reopened or assigned a new ID. An `accepted` criterion
is terminal under its approved exception or human decision. A `passed`
criterion may be evaluated again only when the correction directly affects it;
preserve the prior result and change it to `failed` only when the new evidence
shows that direct failure.
...
- Admit a new blocking finding only when the correction introduces it or it is
  ...
  affected by the correction. A Release block must likewise be tied to a
  frozen roadmap prerequisite or operation check directly affected by the
  correction.
